...
##' @param end Variable defining end of interval
...
generateSeries <- function(data, start, end, step = 1, timeint = "day", varname = "series"){
  ...
  seq.out <- Map(seq, data[, start], data[, end], step)
  ...
  data.out$var <- if(date){as.Date(unlist(seq.out), origin = "1970-01-01")}
  else {unlist(seq.out)}

  ## Add variable name, return output
  names(data.out)[which(names(data.out)=="var")] <- varname
  return(data.out[,!names(data.out)%in% c(start, end)])
}
